...
export default function SubrogationForm({ user }) {
  ...
  const [form, setForm] = useState({
    ...
    claim_amount: "", incident_date: "", originating_office_type: "", originating_office: "",
    received_reference_number: "", date_received: "", remarks: "", priority: "Medium", status: "Submitted",
    subrogation_against: "", responsible_party_contact: "", amount_to_recover: "", subrogation_description: "",
  });
  ...
  const handleSubmit = async (asDraft) => {
    if (!form.claim_reference || !form.claimant_name || !form.insurance_type || !form.claim_amount || !form.incident_date || !form.originating_office_type || !form.originating_office || !form.subrogation_against || !form.amount_to_recover) {
    ...
    try {
      ...
      toast({ title: asDraft ? "Draft saved" : "Subrogation claim registered", description: asDraft ? "You can register it later." : "Subrogation claim has been registered." });
      navigate("/");
    } catch {
  ...
  return (
    <>
      <Card className="border-0 shadow-sm">
        ...
        <CardContent className="space-y-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Amount Paid to Insured (ETB) *</Label>
      ...
      <Card className="border-0 shadow-sm">
        <CardHeader><CardTitle className="text-base">Subrogation Details</CardTitle></CardHeader>
        <CardContent className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Subrogation Against (Responsible Party) *</Label>
              <Input value={form.subrogation_against} onChange={(e) => handleChange("subrogation_against", e.target.value)} placeholder="Third party responsible for the loss" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Responsible Party Contact</Label>
              <Input value={form.responsible_party_contact} onChange={(e) => handleChange("responsible_party_contact", e.target.value)} placeholder="Address / phone / insurance info" />
            </div>
            <div className="space-y-1.5">
              <Label className="text-xs font-medium">Amount to Recover (ETB) *</Label>
              <Input type="number" value={form.amount_to_recover} onChange={(e) => handleChange("amount_to_recover", e.target.value)} placeholder="0.00" />
            </div>
          </div>
          <div className="space-y-1.5">
            <Label className="text-xs font-medium">Subrogation Description</Label>
            <Textarea value={form.subrogation_description} onChange={(e) => handleChange("subrogation_description", e.target.value)} rows={3} placeholder="Describe the subrogation basis and recovery plan..." />
          </div>
        </CardContent>
      </Card>
      ...
      <div className="flex items-center gap-3 justify-end">
        ...
        <Button onClick={() => handleSubmit(false)} disabled={submitting || !!existingClaim} className="gap-2">
          <Save className="w-4 h-4" /> {submitting ? "Registering..." : "Register Subrogation Claim"}
        </Button>
      </div>
    </>
  );
